...
def main():
    ...
    """
    print(
        f"{Fore.YELLOW}{Back.BLUE}{Style.BRIGHT} OLAP CUBES GENERATION FOR MONGODB {Style.RESET_ALL}"
    )
    print(
        f"{Fore.YELLOW}Starting OLAP cubes generation for MongoDB from Northwind data...{Style.RESET_ALL}"
    )
    ...
    if all_valid:
        ...
        print(
            f"{Fore.MAGENTA}To import these CSV files into MongoDB, you can use the following commands:{Style.RESET_ALL}"
        )
        print(
        ...
        print(
            f"{Fore.CYAN}mongoimport --db northwind_olap --collection cube2 --type csv --headerline --file northwind_csv_cubes/cube2.csv{Style.RESET_ALL}"
        )
        print(
            f"{Fore.CYAN}mongoimport --db northwind_olap --collection cube3 --type csv --headerline --file northwind_csv_cubes/cube3.csv{Style.RESET_ALL}"
        )
        print(
            f"{Fore.CYAN}mongoimport --db northwind_olap --collection cube4 --type csv --headerline --file northwind_csv_cubes/cube4.csv{Style.RESET_ALL}"
        )

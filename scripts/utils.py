...
def download_northwind_db(db_path):
    ...
    """
    print(f"{Fore.YELLOW}Northwind database not found. Downloading it now...{Style.RESET_ALL}")
    ...
    try:
        ...
        print(f"{Fore.GREEN}Successfully downloaded Northwind database to {db_path}{Style.RESET_ALL}")
        return True
...
def connect_to_northwind():
    ...
    if not db_path.exists():
        ...
        if not success:
            raise FileNotFoundError(f"{Fore.RED}Could not download Northwind database. Please download it manually and place it at {db_path}{Style.RESET_ALL}")
